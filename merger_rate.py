...
def lc_mrate(M0, z, xi=np.linspace(0.1, 0.9, 100), prescription1=True, sig8=cp.sigma8, h=cp.h, kmax=30, window='TopHat',
             prec=1000, om0=cp.om,
...
def sph_mrate_per_n(M0, z, xi=np.linspace(0.1, 0.9, 100), sig8=cp.sigma8, h=cp.h, kmax=30, window='TopHat', prec=1000, om0=cp.om,
                    ol0=cp.oml, omb=cp.omb, camb=False, colos=True):
    ...
    dwdz = (cp.delta_c(z + 0.0001 * z, om0, ol0) - cp.delta_c(z - 0.0001 * z, om0, ol0)) / (0.0002 * z)
...
def ell_mrate_per_n(M0, z, xi=np.linspace(0.1, 0.9, 100), sig8=cp.sigma8, h=cp.h, kmax=30, window='TopHat', prec=1000, om0=cp.om,
                    ol0=cp.oml, omb=cp.omb, camb=False, colos=True):
...
def integ_mrate(M0, z, xi_min, xi_max, nxibins=10000, mass=False, model='EC', sig8=cp.sigma8, h=cp.h, kmax=30, window='TopHat',
                prec=1000, om0=cp.om, ol0=cp.oml, omb=cp.omb, camb=False, colos=True):
