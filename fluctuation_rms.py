...
def power_spectrum_a2(k, sigma8=sigma8, h=h, om0=om, omb=omb, ns=ns, test=False):
    if test:
        return k**ns*Transfer(k*h, om0, omb, h)**2
    else:
        val_8 = sigma_R(8, sigma8, h, omb, om0, test=True)
        return (sigma8/val_8)**2*k**ns*Transfer(k*h, om0, omb, h)**2
...
def sigma_R(R, sig8=sigma8, h=h, omb=omb, om0=om, ol0 = oml, ns=ns, kmax=30,  prec=1000, window=W_th, camb=False,
    ...
    if camb:
        ...
        prec=10*prec
        k = np.logspace(-7, 5, prec)
        pk = power_spectrum_a2(k, sig8, h, om0, omb, ns, test)
        if type(R) == list or type(R) == np.ndarray:
            # In units of Mpc/h
            n = len(R)  # size of Mass imput
            m = prec  # size of wavenumbers imput
